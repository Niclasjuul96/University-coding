...
import dtu.library.app.LibraryAppException;
...
public class User {
	...
	public void canBorrow(Calendar borrowDate) throws LibraryAppException {
		if (borrowedMedia.size() >= MAX_NUMBER_OF_BOOKS) {
			throw new LibraryAppException(String.format("Can't borrow more than %d books/CDs",MAX_NUMBER_OF_BOOKS));
		}
		if (hasOverdueMedia(borrowDate)) {
			throw new LibraryAppException("Can't borrow book/CD if user has overdue books/CDs");
		}
		if (getHasFine()) {
			throw new LibraryAppException("Can't borrow book/CD if user has outstanding fines");
		}
	}

	public double calculateFine(Calendar currentDate) {
		if (!hasFine) {
